...
class SearchDestinationCubit extends Cubit<SearchDestinationState> {
  ...
  Future<void> fetchPolylinePoints(LatLng destination) async {
    Position source = await Geolocator.getCurrentPosition();
    var url =
        "${GoogleMapEndpoints.placeLatLangUrl}origin=${source.latitude},${source.longitude}&destination=${destination.latitude},${destination.longitude}&${GoogleMapEndpoints.directionUrlInfo}";
  }
