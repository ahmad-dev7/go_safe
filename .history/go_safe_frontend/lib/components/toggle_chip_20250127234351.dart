      ...
      builder: (context, state) {
        ...
        return Stack(
          children: [
            ...
            VxBox().color(activeTextColor).border(color: AppColors.secondary(context)).withRounded().make(),
            //* Aligns to active state
            AnimatedAlign(
              duration: kDuration,
              alignment: state is LoginState ? Alignment.centerLeft : Alignment.centerRight,
              child: VxBox().color(AppColors.accent(context)).width(totalWidth * 0.5).withRounded().make(),
            ),
            //* Login button
            Align(
