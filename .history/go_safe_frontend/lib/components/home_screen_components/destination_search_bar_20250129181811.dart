    ...
    return BlocBuilder<SearchDestinationCubit, SearchDestinationState>(
      builder: (context, state) {
        ...
        return FloatingSearchBar(
          ...
          height: 50,
          title: state is SearchDestinationPicked && label != null ? label!.text.make() : null,
          ...
          onQueryChanged: (query) {
            if (query == '') {
              predictions.clear();
              //! state rebuild is required to let the widget tree know that predictions is cleared
            } else {
              context.read<SearchDestinationCubit>().searchPredictions(query);
            }
          },
          actions: [
            FloatingSearchBarAction.searchToClear(
              showIfClosed: label == null,
              color: AppColors.secondary(context),
            ),
          ],
          builder: (ctx, transition) {
            //? Loader is already shown on top of stack directly
            ...
            return ClipRRect(
              ...
              child: Material(
                color: AppColors.accent(context).withOpacity(.3),
                elevation: 4.0,
                child: Column(
