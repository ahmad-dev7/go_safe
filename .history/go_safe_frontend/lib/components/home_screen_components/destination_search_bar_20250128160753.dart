      ...
      leadingActions: [
        FloatingSearchBarAction(
          ...
          builder: (BuildContext context, Animation<double> animation) {
            ...
            // Determine the current icon based on the search bar's state
            bool isOpen = bar.isOpen;
            ...
            return AnimatedSwitcher(
              ...
              child: IconButton(
                key: ValueKey(isOpen), // Switch between icons when state changes
                icon: Icon(
                  isOpen ? Icons.arrow_back : Icons.map,
                  color: Colors.black,
                  size: 24.0,
                ),
                tooltip: isOpen ? MaterialLocalizations.of(context).backButtonTooltip : "Map", // Tooltip for map icon
                onPressed: () {
                  if (isOpen && !bar.isAlwaysOpened) {
                    bar.close(); // Close the search bar
                  } else {
                    // Map icon action
